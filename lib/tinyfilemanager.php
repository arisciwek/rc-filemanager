...
if ($use_auth) {
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
        ...
        // Form
        unset($_SESSION[FM_SESSION_ID]['logged']);
        // [LOCAL PATCH rc-filemanager] nama field acak + ROTASI TOKEN
        // per render (single-use utk login; form post-auth tetap memakai
        // token sesi yang berlaku)
                                ...
                                <form class="form-signin" action="" method="post" autocomplete="off">
                                    <div class="mb-3">
                                        <!-- [LOCAL PATCH rc-filemanager]
                                             logo & label login dari config
                                             'client_login' (bukan SVG TFM) -->
                                        <div class="brand text-center">
                                            <img src="<?php echo fm_enc(fm_client_login_logo_url()); ?>"
                                                 alt="Logo" style="max-width:100%;height:auto;">
                                        </div>
                                        <div class="text-center">
                                            <h1 class="card-title"><?php
                                                echo fm_enc(fm_client_login_cfg('title', APP_TITLE));
                                            ?></h1>
                                            <?php $welcome = fm_client_login_cfg('welcome');
                                            if ($welcome !== ''): ?>
                                            <p class="text-muted mb-0"><?php echo fm_enc($welcome); ?></p>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                    <hr />
...
{
    $cfg = defined('FILEMANAGER_CLIENT_LOGIN') && is_array(FILEMANAGER_CLIENT_LOGIN)
        ? FILEMANAGER_CLIENT_LOGIN : [];
    return (isset($cfg[$key]) && $cfg[$key] !== '') ? $cfg[$key] : $default;
}

/** Logo login sebagai DATA URI base64 — kebal masalah path/URL.
 *  File custom dari config; bila tidak ada/tak terbaca -> default. */
function fm_client_login_logo_url()
{
    $custom = ltrim(fm_client_login_cfg('logo'), '/');
    $file = $custom !== '' ? $custom : 'skins/elastic/images/logo.png';
    // tinyfilemanager.php ada di lib/ — root plugin satu level di atas
    $abs = dirname(__DIR__) . '/' . $file;
    if (is_readable($abs)) {
        $ext  = strtolower(pathinfo($abs, PATHINFO_EXTENSION));
        $mime = $ext === 'jpg' ? 'jpeg'
            : (in_array($ext, array('png', 'gif', 'svg', 'webp')) ? $ext : 'png');
        $bin = (string) file_get_contents($abs);
        return 'data:image/' . $mime . ';base64,' . base64_encode($bin);
    }
    // fallback terakhir: URL absolut via static.php
    return '/plugins/filemanager/skins/elastic/images/logo.png';
}
...
?>
    <!-- [LOCAL PATCH rc-filemanager] Breadcrumb dikeluarkan dari <nav>
         agar selalu terlihat (tidak ikut tersembunyi di dalam collapse
         navbar saat iframe sempit) dan menjadi baris tersendiri di atas
         navbar. RE-APPLY patch ini setelah overwrite tinyfilemanager.php
         dengan versi TFM baru. -->
